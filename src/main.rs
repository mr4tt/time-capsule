...
fn main() {
    ...
    for path in paths {
        ...
        let send_date = NaiveDate::parse_from_str(file_name.split(".").next().unwrap(), "%Y-%m-%d");

        match send_date {
            Ok(_) => {
                if send_date.unwrap() == curr_date {
                    println!("found a match!");
                    send_email(format!("{curr_path}letters/{file_name}"), &curr_path);
                }
            }
            Err(_) => { continue; }
        };
...
fn send_email(file_name: String, env_path: &str) {
    ...
    let email = Message::builder()
        ...
        .subject("Unearthed Time Capsule")
